...
* updated : 在重新渲染頁面後叫用，這時的頁面已經被重渲染為改變後的畫面。
...
這個例子在之前的文章中也有使用，只有一個很單純 a 資料以及綁定 Vue 實體到 #app 的設定。
...
* create : 到了 create 時已經創建實例，所以 a 已變為 1 ，但是 $el 因為還未掛載至目標元素，所以依然是 undefined 。
...
* beforeMount : 流程圖上有提到，在叫用 beforeMount 前 Vue 已經決定模板的樣式，所以在 beforeMount 中的 $el 已經有值了，只是它還未依照 Vue 實體上的定義所渲染，只是個初始設定的模板。
* mounted : 在 mounted 被叫用時已經把 Vue 實體上的定義綁定到元素上，所以這裡看到的是經由 Vue 渲染後的配置。

> 所以在 beforeMount 前不能使用 $el 操作 DOM 元素。
...
* beforeUpdate : a 改變後觸發 beforeUpdate ，可以看到 a 已經變為2了，可是頁面上還是1 ，表示這時雖然 data 已經改變，可是還沒有重新渲染畫面。
* updated : 完成重新渲染的作業後觸發，這時可以看到畫面已經將1改為2了。
...
* beforeDestroy : 叫用 beforeDestroy 表示即將執行銷毀動作，如果有些物件要釋放資源可以在這裡處理。
...
有了本篇的知識，在開發時對於要修改資料或是操作 DOM 的時機應該可以掌握得更好。
